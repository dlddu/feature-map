 ...
 *
 * 테스트 대상: src/app/api/settings/api-keys/route.ts
 *
 * Mock 전략:
...
describe("GET /api/settings/api-keys", () => {
  beforeEach(() => {
    ...
    mockDecrypt.mockImplementation((encrypted: string) => {
      if (encrypted === "encrypted-openai-key") return "sk-openai-real-key-abcd";
      if (encrypted === "encrypted-anthropic-key") return "sk-ant-real-key-wxyz";
  ...
  describe("성공 케이스 (happy path)", () => {
    ...
    it("각 키에 id, provider, maskedKey, label, isActive가 포함된다", async () => {
      const request = makeRequest();
      const response = await GET(request);
      const body = await response.json();
    ...
    it("maskedKey가 maskApiKey 반환값으로 설정된다", async () => {
      ...
      expect(body.apiKeys[0].maskedKey).toBe("sk-...abcd");
      expect(body.apiKeys[1].maskedKey).toBe("sk-...wxyz");
    });
    ...
    it("decrypt가 각 키의 encryptedKey로 호출된다", async () => {
      const request = makeRequest();
      await GET(request);
  ...
  describe("복호화 실패 케이스", () => {
    ...
    it("모든 키의 복호화가 실패해도 200과 빈 배열을 반환한다", async () => {
