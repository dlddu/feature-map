...
test.describe("대시보드: 연결된 레포 카드 목록 표시", () => {
  ...
  test("로그인한 사용자가 대시보드에 접근하면 연결된 레포 카드 목록이 표시된다", async ({
    page,
  }) => {
  ...
  test("대시보드에 레포 카드가 표시될 때 레포명과 기본 브랜치 정보가 함께 표시된다", async ({
    page,
  }) => {
...
test.describe("대시보드: 레포 카드 상세 정보 표시", () => {
  ...
  test("레포 카드에 feature 수가 표시된다", async ({ page }) => {
  ...
  test("레포 카드에 분석 상태(PENDING/IN_PROGRESS/COMPLETED/FAILED)가 표시된다", async ({
    page,
  }) => {
  ...
  test("레포 카드에 마지막 분석 시각이 표시된다", async ({ page }) => {
...
test.describe("대시보드: 레포 카드 클릭 → 레포 상세 이동", () => {
  ...
  test("레포 카드를 클릭하면 해당 레포 상세(개요) 페이지로 이동한다", async ({
    page,
  }) => {
  ...
  test("레포 상세 개요 페이지에 레포명과 개요 정보가 표시된다", async ({
    page,
  }) => {
...
test.describe("대시보드: 레포 없을 때 빈 상태 + CTA 표시", () => {
  ...
  test("연결된 레포가 없는 사용자의 대시보드에 빈 상태 안내 문구가 표시된다", async ({
    page,
  }) => {
  ...
  test("연결된 레포가 없는 사용자의 대시보드에 레포 연결 CTA 버튼이 표시된다", async ({
    page,
  }) => {
    // Arrange: e2e-test-user-003은 레포 없이 시딩된 유저
  ...
  test("빈 상태에서 레포 연결 CTA 버튼을 클릭하면 레포 선택 다이얼로그가 열린다", async ({
    page,
  }) => {
    // Arrange: e2e-test-user-003은 레포 없이 시딩된 유저
