...
test.describe("레포 상세 개요 패널: 파이프라인 단계(F1~F5) 상태 표시", () => {
  ...
  test("레포 상세 개요 패널 진입 시 F1~F5 파이프라인 단계 목록이 표시된다", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("[data-testid='pipeline-step-f1']")
    ).toBeVisible();

    // Assert: F2 단계가 표시되어야 한다 (전략 분석)
    await expect(
      page.locator("[data-testid='pipeline-step-f2']")
    ).toBeVisible();

    // Assert: F3 단계가 표시되어야 한다 (Feature 추출)
    await expect(
      page.locator("[data-testid='pipeline-step-f3']")
    ).toBeVisible();

    // Assert: F4 단계가 표시되어야 한다 (Feature 분류)
    await expect(
      page.locator("[data-testid='pipeline-step-f4']")
    ).toBeVisible();

    // Assert: F5 단계가 표시되어야 한다 (인수 테스트 생성)
    await expect(
      page.locator("[data-testid='pipeline-step-f5']")
    ).toBeVisible();
  ...
  test("각 파이프라인 단계(F1~F5)에 현재 상태 뱃지가 표시된다", async ({
    page,
  }) => {
    ...
    const f1Step = page.locator("[data-testid='pipeline-step-f1']");
    await expect(
...
test.describe('레포 상세 개요 패널: "전체 실행" → 순차 실행 및 SSE 상태 업데이트', () => {
  ...
  test('"전체 실행" 버튼을 클릭하면 F1 단계부터 실행이 시작된다', async ({
    page,
  }) => {
  ...
  test("SSE로 파이프라인 단계별 상태 변경이 실시간으로 반영된다", async ({
    page,
  }) => {
...
test.describe('레포 상세 개요 패널: 개별 "▶" 버튼으로 단계 실행', () => {
  ...
  test('F1 단계의 "▶" 버튼을 클릭하면 F1 단계만 실행된다', async ({
    page,
  }) => {
    ...
    const f2Step = page.locator("[data-testid='pipeline-step-f2']");
    await expect(
  ...
  test('선행 단계(F1)가 완료되지 않은 상태에서 F2의 "▶" 버튼을 클릭하면 경고가 표시된다', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("[data-testid='prerequisite-warning']")
...
test.describe('레포 상세 개요 패널: "중단" 버튼으로 파이프라인 실행 중단', () => {
  ...
  test('"중단" 버튼을 클릭하면 현재 실행 중인 파이프라인이 중단된다', async ({
    page,
  }) => {
...
test.describe("레포 상세 개요 패널: QuickStats 카드 탭 → 섹션 탭 전환", () => {
  ...
  test("QuickStats의 Feature 수 카드를 탭하면 Features 섹션 탭으로 전환된다", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("[data-testid='quick-stats-grid']")
    ).toBeVisible();
    ...
    await expect(
      page.locator("[data-testid='section-tab-features'][aria-selected='true']")
    ).toBeVisible();

    // Assert: Features 패널 콘텐츠가 표시되어야 한다
    await expect(
      page.locator("[data-testid='features-panel']")
    ).toBeVisible();
  });

  // skip: QuickStatsGrid 컴포넌트 미구현 (DLD-621)
  test("QuickStats의 계층 수 카드를 탭하면 계층 섹션 탭으로 전환된다", async ({
    page,
  }) => {
    // Act: 레포 상세 개요 페이지로 이동
    await page.goto("/repos/e2e-test-repo-001/overview");
    ...
    await expect(
      page.locator("[data-testid='section-tab-layers'][aria-selected='true']")
    ).toBeVisible();
  });
});
...
test.describe("레포 상세: SectionTabs로 개요/계층/전략/Features 패널 전환", () => {
  ...
  test("SectionTabs에서 '계층' 탭을 클릭하면 계층 패널로 전환된다", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("[data-testid='section-tabs']")
    ).toBeVisible();

    // Assert: 기본적으로 "개요" 탭이 활성화되어야 한다
    await expect(
      page.locator("[data-testid='section-tab-overview'][aria-selected='true']")
    ).toBeVisible();

    // Act: "계층" 탭 클릭
    await page
      .locator("[data-testid='section-tab-layers']")
      .click();

    // Assert: 계층 탭이 활성화(aria-selected=true)되어야 한다
    await expect(
      page.locator("[data-testid='section-tab-layers']")
  ...
  // skip: SectionTabs 컴포넌트 미구현 (DLD-621)
  test("SectionTabs에서 '전략' 탭을 클릭하면 전략 패널로 전환된다", async ({
    page,
  }) => {
    // Act: 레포 상세 개요 페이지로 이동
    await page.goto("/repos/e2e-test-repo-001/overview");
    ...
    // Act: "전략" 탭 클릭
    await page
      .locator("[data-testid='section-tab-strategy']")
      .click();

    // Assert: 전략 탭이 활성화되어야 한다
    await expect(
      page.locator("[data-testid='section-tab-strategy']")
    ).toHaveAttribute("aria-selected", "true");

    // Assert: 전략 패널 콘텐츠가 표시되어야 한다
    await expect(
      page.locator("[data-testid='strategy-panel']")
    ).toBeVisible();
  });

  // skip: SectionTabs 컴포넌트 미구현 (DLD-621)
  test("SectionTabs에서 'Features' 탭을 클릭하면 Features 패널로 전환된다", async ({
    page,
  }) => {
    // Act: 레포 상세 개요 페이지로 이동
    await page.goto("/repos/e2e-test-repo-001/overview");
    ...
    // Act: "Features" 탭 클릭
    await page
      .locator("[data-testid='section-tab-features']")
      .click();

    // Assert: Features 탭이 활성화되어야 한다
    await expect(
      page.locator("[data-testid='section-tab-features']")
    ).toHaveAttribute("aria-selected", "true");
  ...
  // skip: SectionTabs 컴포넌트 미구현 (DLD-621)
  test("SectionTabs에서 '개요' 탭을 클릭하면 개요 패널로 돌아온다", async ({
    page,
  }) => {
    // Act: 레포 상세 개요 페이지로 이동
    await page.goto("/repos/e2e-test-repo-001/overview");
    ...
    await page
      .locator("[data-testid='section-tab-overview']")
      .click();

    // Assert: 개요 탭이 활성화되어야 한다
    await expect(
      page.locator("[data-testid='section-tab-overview']")
    ).toHaveAttribute("aria-selected", "true");
